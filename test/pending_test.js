suite('collector.pending', () => {
  ...
  test('at startup, unpaired task runs are measured, but that stops once a pair is seen', async () => {
    fakeTaskChange({state: 'running', taskId: 't1', scheduled: fakes.clock.msec() - 7000, started: fakes.clock.msec()});
    fakeTaskChange({state: 'pending', taskId: 't3', scheduled: fakes.clock.msec()});
    await fakes.clock.tick(10000);
    fakeTaskChange({state: 'running', taskId: 't2', scheduled: fakes.clock.msec() - 8000, started: fakes.clock.msec()});
    await fakes.clock.tick(1000);
    fakeTaskChange({state: 'running', taskId: 't3', scheduled: fakes.clock.msec() - 11000, started: fakes.clock.msec()});
    await fakes.clock.tick(1000);
    fakeTaskChange({state: 'running', taskId: 't4', scheduled: fakes.clock.msec() - 9000, started: fakes.clock.msec()});
    await fakes.clock.tick(1000);
    assertMeasures({
